...
Kurse sind die Quelle für akademische Profilattribute. Ein Kurs besteht aus `code` (z. B. `TIF25A`), `studyProgram`, `isActive` und `createdAt`. Registrierung und Profiländerung senden nur den Kurscode; der Studiengang wird serverseitig aus dem Kurskatalog übernommen. Die öffentliche Kursliste `/api/courses` liefert nur Studienkurse. Die Admin-Kursliste `/api/admin/courses` enthält zusätzlich die Systemkurse `ADMIN`, `LECTURER` und `MANAGEMENT`, damit Admins, Lehrende und Verwaltung ebenfalls eine Kurszuordnung erhalten können.
